...
If these rules are violated, the response is considered INVALID`,
	...
	try {
		...
		const normalizedPages = result.pages.map((item, index) =>
			normalizePathAndTitle(item, index),
		);
				...
				pages: [
					{
						...
						title: 'Home',
						seo:
							normalizedPages[0]?.seo ||
							createFallbackSEO(prompt, language, country, 'Home'),
					},
				],
				hasError: false,
				tokens: {
...
const createFallbackSEO = (prompt, language, country, pageName) => {
	return {
		...
		description:
			`${prompt.substring(0, 140)}. Serving ${country} with quality services.`.substring(
				0,
				160,
			),
		...
		ogDescription:
			`${prompt.substring(0, 140)}. Professional services in ${country}.`.substring(0, 160),
	};
};
